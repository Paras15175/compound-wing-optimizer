...
CRUISE_CL_MIN = 0.1  # Value of Cl at very low AoA for most cambered airfoils
...
base_x_le_tip = 0.25 * (ROOT_CHORD - base_tip_chord)  # ASSUMPTION - taper about quarter chord for stability
...
# Calculate the Cl required in base config, so L/D is not artificially inflated by generating more lift when we dont need it
...
if max_possible_area < S_min_required:
    required_half_span = (S_min_required / (2 * effective_max_root_chord)) * 1.2
...
opti_area = 2 * ((opti_root_chord * opti_rect_span)+ (0.5 * (opti_root_chord + opti_tip_chord) * (opti_half_span - opti_rect_span)))
